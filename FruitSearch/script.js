...
function search(str) {
	...
	/* results = fruit.filter(function(ele,index){
		return ele.toLowerCase().indexOf(str.toLowerCase())
	})

	results = results.map(function(ele){
		let foundIn = ele.toLowerCase().indexOf(str.toLowerCase());
		return {
			'matchingFruit': ele,
			'startMatchingSearchTerm':foundIn,
			'endMatchingSearchTerm':foundIn+str.length-1
		};
	}) */

	//for loops much slower than above
	//for let of is the elements instead of index
	for(let index in fruit){
		//if search is found within a fruit in fruit array foundIn = first index, otherwise foundIn = -1
		//notes: do this outside of the array once. dry principle = do not repeat yourself
...
function searchHandler(e) {
	...
	if(input.value===""){//if !input.value instead. learn difference between truthy and falsy
		document.querySelector('#suggestionsList').innerHTML="";
	}
	//only show results if input is not empty
	else{
		if(e.target.getAttribute('id')=='fruit' & input.value!==""){//maybe we don't need this 'id'==fruit if statement
...
function showSuggestions(results, inputVal) {
	...
	//can use advanced array methods to replace nested for loops
	for(let resultIndex in results){
		...
		for(let letterIndex in resultStringArray){//use slice or replace instead. or look at regex
